...
[![Version](https://img.shields.io/badge/Version-1.22.1-purple)](CHANGELOG.md)
...
7. Pseudocode blocks prohibited; executable examples and reader-friendly Mermaid infographics remain allowed

Verified reference titles and URLs are extracted and appended by local code instead of being generated by the model. Before release validation, local code removes any model-generated pseudocode block without spending another API call. A local validator blocks missing metadata, broken code fences, insufficient structure, and severely truncated content.
...
**v1.22.1** — Keeps stable Obsidian revision block targets valid after earlier block deletions in the same section.
...
- **`[ ]` v1.22.2**: Evaluate Gemini structured output after the graceful release policy is proven in production
