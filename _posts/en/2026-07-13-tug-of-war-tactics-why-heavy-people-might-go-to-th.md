---
layout: post
title: "Tug-of-War Tactics: Why Heavy People Might Go to the Back (The Physics Behind the Pull)"
...
```mermaid
graph TD
    A["Rope End (Opponent's Side)"]
    B["Player 1 (Lightest/Agile)"]
    C["Player 2 (Lighter)"]
    D["Player 3 (Medium)"]
    E["Player 4 (Heavier)"]
    F["Player 5 (Heaviest/Anchor)"]
    G["Rope End (Our Side)"]
    ...
    C -- "Transmits Force" --> D
    D -- "Transmits Force" --> E
    E -- "Transmits Force & Anchors" --> F
    F -- "Ultimate Anchor" --> G

    subgraph Gradient Team Structure
        B --> C
        C --> D
        D --> E
E --> F
