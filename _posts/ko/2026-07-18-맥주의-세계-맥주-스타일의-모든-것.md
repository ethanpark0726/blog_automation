...
맥주는 크게 사용된 효모의 종류와 발효 온도에 따라 두 가지 주요 범주로 나뉩니다. 상면 발효(Top-fermentation)는 효모가 발효조 상단에서 활동하는 방식으로 주로 에일 맥주에 사용되며, 하면 발효(Bottom-fermentation)는 효모가 하단으로 가라앉으며 발효되는 방식으로 라거 맥주에 사용됩니다.
...
*   **예시:** 필스너(Pilsner), 헬레스(Helles), 복(Bock). 여기서 '복(Bock)'은 독일의 전통적인 강한 라거 스타일을 의미하며, 샤이너 복(Shiner Bock)은 이 스타일을 대중적으로 재해석한 대표적인 맥주 중 하나입니다.
...
### IPA (India Pale Ale)
IPA는 현대 수제 맥주 열풍의 핵심입니다. 흔히 영국 식민지 시대의 무역과 연관 지어 설명되곤 하는데, 여기서 '인디아(India)'는 실제 국가 인도에서 유래했다기보다는 당시 영국에서 인도로 맥주를 수출할 때 긴 항해 기간 동안 변질을 막기 위해 홉을 다량 첨가했던 역사적 배경에서 이름 붙여진 것입니다. 정확한 명칭은 **인디아 페일 에일(India Pale Ale)**이라는 점을 기억하세요.
*   **풍미 프로필:** 사용하는 홉의 종류에 따라 강렬한 쓴맛, 감귤류, 솔 향, 혹은 열대 과일 향이 지배적입니다.

### 스타우트(Stout)
스타우트는 볶은 몰트의 풍미가 특징인 어둡고 불투명한 에일입니다. 역사적으로는 '포터(Porter)' 스타일과 밀접한 관련이 있습니다. 포터는 18세기 런던에서 유래한 어두운 맥주였고, 스타우트는 원래 '스타우트 포터(Stout Porter)'라 불리던 더 강하고 진한 포터를 지칭했습니다. 즉, 스타우트는 포터의 더 강한 버전에서 시작되었으나, 오늘날에는 두 스타일이 서로 다른 개성을 가진 독립적인 범주로 자리 잡았습니다.
*   **풍미 프로필:** 커피, 초콜릿, 볶은 곡물의 향이 느껴집니다.

### 라이트 라거(Light Lager)와 미켈롭 울트라
미켈롭 울트라(Michelob Ultra)와 같은 맥주는 '라이트 라거(Light Lager)'로 광고됩니다. 여기서 '라이트'는 단순히 도수가 낮은 것만을 의미하지 않고, 칼로리와 탄수화물 함량이 낮은 맥주를 뜻합니다. 일반적인 맥주는 355ml 한 캔당 약 150kcal 내외를 함유하는데, 미켈롭 울트라는 발효 과정에서 효모가 당분을 거의 완전히 분해하도록 유도하여 잔류 당분을 최소화함으로써 낮은 칼로리를 구현합니다.

## 비교표: 한눈에 보는 맥주 스타일

| 스타일 | 발효 방식 | 주요 풍미 | 색상 |
| :--- | :--- | :--- | :--- |
| **라거** | 하면 발효 | 깔끔함, 청량감, 빵 향 | 연한 짚색 ~ 황금색 |
| **IPA** | 상면 발효 | 홉 향, 쓴맛, 감귤류 | 황금색 ~ 구리색 |
| **스타우트** | 상면 발효 | 커피, 초콜릿, 로스팅 | 짙은 갈색 ~ 검은색 |
| **밀맥주** | 상면 발효 | 바나나, 정향, 빵 향 | 연한 색 ~ 탁한 색 |
